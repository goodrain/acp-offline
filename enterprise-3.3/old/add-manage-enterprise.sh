...
if [ -n "$RELEASE_INFO" ];then
    ...
    if [ $item_length -eq 2 ];then
        ...
        curl -I repo.goodrain.com/install/$dist_name/$dist_version/enterprise/manage.sh 2>/dev/null | tee | head -1 | grep '200 OK' >/dev/null
        if [ $? -eq 0 ];then
            exec curl repo.goodrain.com/install/$dist_name/$dist_version/enterprise/manage.sh 2>/dev/null | bash
        else
