...
# set local install flag
[ ! -d /etc/goodrain ] && mkdir /etc/goodrain

if [ -f /etc/goodrain/.inited ];then
  if [ ! $(grep install /etc/goodrain/.inited) ];then
    echo "install=local" >>/etc/goodrain/.inited
  fi
else
  echo "install=local" >>/etc/goodrain/.inited
fi
